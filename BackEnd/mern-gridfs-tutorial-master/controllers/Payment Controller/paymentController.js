...
router.post("/specific/question/:questionId", (req, res) => {
 ...
 router.post("/:id", (req, res) => {
    const id = req.params.id;
    PaymentSchema.findById(id, (err, project) => {
      if (!project) {
        res.status(404).send("question not found");
      } else {

        project.studentName = req.body.studentName,
        project.studentID = req.body.studentID,
        project.payableAmount = req.body.payableAmount,
        project.amountPaid = req.body.amountPaid,
        project.datePaid = req.body.datePaid,
